...
What it does:
  ...
  2. Copies Content/Python/uefn_tools/ into your project's Content/Python/
  3. Creates init_unreal.py if one doesn't exist, or patches it if one does

After running:
  Open UEFN. The Toolbelt menu appears automatically.
  In the Python console: import uefn_tools as tb; tb.smoke_test()
"""
...
TOOLBELT_SRC = os.path.join(REPO_ROOT, "Content", "Python", "uefn_tools")
...
def _install_toolbelt(project_path: str):
    ...
    dest_tb     = os.path.join(dest_python, "uefn_tools")
    ...
    try:
        ...
        print(f"  ✓ Copied uefn_tools → {dest_tb}")
...
Next steps:
  ...
  3. Verify in the Python console:
       ...
       import uefn_tools as tb; tb.register_all_tools(); tb.run("toolbelt_smoke_test")

To write your own plugin:
  1. Create a .py file with a @register_tool decorated function
  2. Drop it in:
       [Project]/Saved/uefn_tools/Custom_Plugins/
