...
mount.** The Toolbelt always lives at `[Project]/Content/Python/uefn_tools/...`, so this
...
This works because every tool file lives at `[Project]/Content/Python/uefn_tools/tools/toolname.py`. Walking four levels up always reaches `Content/`.
...
```python
import sys; [sys.modules.pop(k) for k in list(sys.modules) if "uefn_tools" in k]; import uefn_tools as tb; tb.register_all_tools(); ...
```

forcibly removes all `uefn_tools` modules from `sys.modules`, which triggers Python's
...
```python
import uefn_tools as tb; tb.register_all_tools()
```
...
```python
import uefn_tools as tb; tb.register_all_tools(); tb.run("verse_graph_open")
```
