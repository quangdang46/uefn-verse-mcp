...
:: ── Copy files ────────────────────────────────────────────────────────────────
echo  [1/4]  Copying uefn_tools package...
xcopy /E /I /Y "%~dp0Content\Python\uefn_tools" "!DEST!\Content\Python\uefn_tools" >nul
...
echo import sys; [sys.modules.pop(k) for k in list(sys.modules) if "uefn_tools" in k]; import uefn_tools as tb; tb.register_all_tools(); tb.launch_qt()
echo.
echo [SMOKE TEST] Reload + verify health:
echo import sys; [sys.modules.pop(k) for k in list(sys.modules) if "uefn_tools" in k]; import uefn_tools as tb; tb.register_all_tools(); tb.run("toolbelt_smoke_test")
echo.
echo [INTEGRATION TEST] WARNING - invasive, use a clean template only:
echo import sys; [sys.modules.pop(k) for k in list(sys.modules) if "uefn_tools" in k]; import uefn_tools as tb; tb.register_all_tools(); tb.run("toolbelt_integration_test")
echo.
echo [MASTER SYNC] Reload + sync docs ^& Verse IQ:
echo import sys; [sys.modules.pop(k) for k in list(sys.modules) if "uefn_tools" in k]; import uefn_tools as tb; tb.register_all_tools(); tb.run("api_sync_master")
echo.
echo [VERSE GRAPH] Reload + open device graph:
echo import sys; [sys.modules.pop(k) for k in list(sys.modules) if "uefn_tools" in k]; import uefn_tools as tb; tb.register_all_tools(); tb.run("verse_graph_open")
echo.
echo ==========================================
