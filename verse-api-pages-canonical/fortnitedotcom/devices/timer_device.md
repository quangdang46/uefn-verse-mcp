...
---|---
[`ApplyDisguise`](https://dev.epicgames.com/documentation/fortnite/verse-api/fortnitedotcom/devices/disguise_device/applydisguise) |  Applies the disguise to the provided `player`. If the provided `player` does not pass the device's filter settings, or if another disguise is already present and the device's _Stomp Existing Disguise_ option is set to false, the disguise will not be applied.
