...
Harness Pattern:
  ...
  3. Run uefn_tools tool
  4. Verify the delta (property changed, file created)
  5. Clean up (destroy actors)

Usage:
    import uefn_tools as tb; tb.run("toolbelt_integration_test")
"""
...
from uefn_tools.registry import register_tool
from uefn_tools.core import undo_transaction

# ─── Configuration ────────────────────────────────────────────────────────────

_SAVED = os.path.join(unreal.Paths.project_saved_dir(), "uefn_tools")
...
def _save_report() -> str:
    ...
    lines = [
        "uefn_tools — Integration Test Results",
        "=" * 60,
...
def _test_materials() -> None:
    ...
    try:
        ...
        # Note: If M_uefn_toolsBase is missing, we use a built-in engine material for verification
        PARENT_MATERIAL_PATH = "/uefn_tools/Materials/M_uefn_toolsBase"
...
def _test_asset_tagger() -> None:
    ...
    try:
        ...
        export_path = os.path.join(unreal.Paths.project_saved_dir(), "uefn_tools", "tag_export.json")
...
def _test_splines() -> None:
    ...
    try:
        ...
        export_path = os.path.join(unreal.Paths.project_saved_dir(), "uefn_tools", "spline_export.json")
...
def _test_assets() -> None:
    ...
    try:
        ...
        report_path = os.path.join(unreal.Paths.project_saved_dir(), "uefn_tools", "rename_report.json")
...
def _test_memory() -> None:
    ...
    try:
        ...
        report_path = os.path.join(unreal.Paths.project_saved_dir(), "uefn_tools", "memory_report.json")
        passed = os.path.exists(report_path)
...
def _test_reference_auditor() -> None:
    ...
    try:
        ...
        report_path = os.path.join(unreal.Paths.project_saved_dir(), "uefn_tools", "ref_audit_report.json")
        passed = os.path.exists(report_path)
...
def _test_text_painter() -> None:
    ...
    try:
        ...
        tb.run("text_clear_folder", folder="uefn_toolsText")
        
        remaining = [
            a for a in actor_sub.get_all_level_actors() 
            if isinstance(a, unreal.TextRenderActor) and ("TestText" in str(a.get_folder_path()) or "uefn_toolsText" in str(a.get_folder_path()))
        ]
...
def toolbelt_integration_test(**kwargs) -> None:
    ...
    unreal.log("══════════════════════════════════════════════════════")
    unreal.log("  uefn_tools INTEGRATION TEST — STARTING")
    unreal.log("══════════════════════════════════════════════════════")
    
    with undo_transaction("uefn_tools Integration Test"):
        try:
            _test_materials()
            _test_bulk_ops()
