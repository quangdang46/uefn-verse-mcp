...
named style preset (JSON) in Saved/uefn_tools/text_styles.json and
...
BLUEPRINT:
    "Execute Python Command" →
        import uefn_tools as tb; tb.run("text_place", text="GAME START", color="#FFFFFF", location=(0,0,200))
...
STYLES_FILE = os.path.join(
    unreal.Paths.project_saved_dir(), "uefn_tools", "text_styles.json"
)

TEXT_FOLDER = "uefn_toolsText"
...
def run_text_save_style(
    ...
    **kwargs,
    ...
    """
    Save a style preset to Saved/uefn_tools/text_styles.json.
...
@register_tool(
    ...
    category="Text & Signs",
    description="Delete all TextRenderActors in the uefn_tools text folder (undoable).",
    tags=["text", "clear", "delete", "cleanup"],
)
def run_text_clear_folder(folder: str = TEXT_FOLDER, **kwargs) -> dict:
    """
    Args:
        folder: World Outliner folder to clear (default: uefn_toolsText).
    """
    actor_sub = unreal.get_editor_subsystem(unreal.EditorActorSubsystem)
