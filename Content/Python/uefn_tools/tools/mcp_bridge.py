...
ATTRIBUTION:
    ...
    This implementation is an independent rewrite extended for the uefn_tools stack.
...
Usage:
    # From the uefn_tools console or REPL:
...
def _c_ping() -> dict:
    return {
        "status":         "ok",
        "app":            "UEFN uefn_tools MCP Bridge",
...
def _c_run_tool(tool_name: str, kwargs: dict | None = None) -> dict:
    """
    Run any registered UEFN uefn_tools tool by name.
...
def _c_list_tools(category: str = "") -> dict:
    """List all registered UEFN uefn_tools tools, optionally filtered by category."""
    import uefn_tools as _tb
...
def mcp_start(port: int = 0, **kwargs) -> None:
    """
    Start the UEFN uefn_tools MCP HTTP listener.
