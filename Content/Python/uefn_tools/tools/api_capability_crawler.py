"""
uefn_tools — API Capability Crawler
========================================
...
def crawl_selection(**kwargs) -> dict:
    ...
    # Save to disk
    import unreal
    saved_dir = os.path.join(unreal.Paths.project_saved_dir(), "uefn_tools")
...
def crawl_level_classes(**kwargs) -> dict:
    ...
    for i, cls_name in enumerate(sorted_classes):
        ...
        unreal.log(f"[uefn_tools] Crawling {i+1}/{len(sorted_classes)}: {cls_name}")
...
def api_sync_master(**kwargs) -> dict:
    """
    Unifies all uefn_tools intelligence into one command.
    ...
    md_content.append("\n---\n*Last Sync: Generated by UEFN uefn_tools Master Sync Tool*")
...
@register_tool(
    ...
    example='result = tb.run("world_state_export")  # → Saved/uefn_tools/world_state.json with all actors + transforms + properties',
)
def world_state_export(**kwargs) -> dict:
    ...
    Output: Saved/uefn_tools/world_state.json
    ...
    unreal.log(f"[uefn_tools] world_state_export: capturing {len(all_actors)} actors...")
    ...
    unreal.log(f"[uefn_tools] ✓ World state saved: {out_path}")

    repo_path = _sync_to_repo(out_path, "world_state.json")
    if repo_path:
        unreal.log(f"[uefn_tools] ✓ Auto-synced to repo: {repo_path}")
...
def device_catalog_scan(
    ...
    *should* have, generate the Verse wiring for it, and tell you exactly which
    ...
    Output: Saved/uefn_tools/device_catalog.json
