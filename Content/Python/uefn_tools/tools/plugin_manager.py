"""
uefn_tools — Plugin Management Tools
========================================
...
import unreal
from uefn_tools.registry import register_tool, get_registry
from uefn_tools import core

@register_tool(
    name="plugin_validate_all",
    category="Utilities",
    description="Validate all registered tools against the uefn_tools schema requirements.",
...
def list_custom(**kwargs) -> dict:
    """List tools that were loaded from the Saved/uefn_tools/Custom_Plugins directory."""
    custom_tools = []
    reg = get_registry()
    ...
    if not custom_tools:
        core.log_info("No custom plugins found in Saved/uefn_tools/Custom_Plugins.")
    else:
...
def export_manifest(**kwargs) -> dict:
    ...
    tool_manifest.json to Saved/uefn_tools/.
