...
def run_import_image_from_url(
    ...
    **kwargs
    ...
    if not asset_dir:
        asset_dir = get_config().get("import.default_dir") or f"/{detect_project_mount()}/uefn_tools/Textures"
    if not url.strip():
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "UEFN uefn_tools/1.0.0"})
