...
Output:
  Saved/uefn_tools/tag_export.json
"""
...
_TAG_PREFIX   = "TB:"          # uefn_tools metadata key prefix
...
def _do_tag_show() -> None:
    ...
    for path in paths:
        ...
        if tags:
            ...
            unreal.log(f"  {name}  ← no uefn_tools tags")

    if not any_tags:
        unreal.log("  (none of the selected assets have uefn_tools tags)")
    unreal.log("")
...
@register_tool(
    name="tag_show",
    category="Asset Tagger",
    description="Print every uefn_tools metadata tag on all selected Content Browser assets",
    icon="🔖",
...
def tag_show(**kwargs) -> dict:
    ...
    Assets with no uefn_tools tags are shown with a '← no uefn_tools tags' note.
    """
    _do_tag_show()
    return {"status": "ok"}
...
@register_tool(
    name="tag_list_all",
    category="Asset Tagger",
    description="List every unique uefn_tools tag key used under a folder with asset counts",
    icon="📋",
...
def tag_list_all(folder: str = "/Game", **kwargs) -> dict:
    ...
    if not counts:
        unreal.log(f"[AssetTagger] No uefn_tools tags found under {folder}.")
...
@register_tool(
    name="tag_export",
    category="Asset Tagger",
    description="Export full tag → asset index to JSON (Saved/uefn_tools/tag_export.json)",
    icon="📤",
...
def tag_export(folder: str = "/Game", **kwargs) -> dict:
    ...
    tag → asset mapping to Saved/uefn_tools/tag_export.json.
