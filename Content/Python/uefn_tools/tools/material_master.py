...
BLUEPRINT CALL:
    "Execute Python Command" node → string:
        import uefn_tools as tb; tb.run("material_apply_preset", preset="gold")
...
SETUP:
    ...
    Recommended parent material path (set PARENT_MATERIAL_PATH below):
        /Game/uefn_tools/Materials/M_uefn_toolsBase

    If you don't have this yet, use any UE5 master material with those params,
    or create M_uefn_toolsBase in Unreal's material editor with them exposed.
"""
...
PARENT_MATERIAL_PATH = "/Game/uefn_tools/Materials/M_uefn_toolsBase"
INSTANCE_OUTPUT_PATH = "/Game/uefn_tools/Materials/Instances"
CUSTOM_PRESETS_FILE  = os.path.join(
    unreal.Paths.project_saved_dir(), "uefn_tools", "custom_presets.json"
)
...
def run_apply_preset(preset: str = "chrome", **kwargs) -> dict:
    ...
    if not unreal.EditorAssetLibrary.does_asset_exist(PARENT_MATERIAL_PATH):
        log_error(
            ...
            f"  Create M_uefn_toolsBase in your project or update PARENT_MATERIAL_PATH "
