"""
uefn_tools — Smoke Test
=========================================
Run this inside the UEFN editor to verify the full uefn_tools stack is healthy.

Usage (UEFN Output Log / Python REPL):
    py "<project>/Content/Python/tests/smoke_test.py"

    — or from the uefn_tools REPL:
    import uefn_tools as tb; tb.run("toolbelt_smoke_test")
...
What this checks:
    ...
    Layer 3 — uefn_tools core       (registry, all 24 modules, tool count, output paths)
...
Results are printed to the Output Log and saved to:
    <project>/Saved/uefn_tools/smoke_test_results.txt
"""
...
def _save_results() -> str:
    """Write results to Saved/uefn_tools/smoke_test_results.txt."""
    try:
        import unreal
        saved = os.path.join(unreal.Paths.project_saved_dir(), "uefn_tools")
    except ImportError:
...
def _layer_toolbelt() -> None:
    _header("Layer 3 — uefn_tools Core")

    try:
        import uefn_tools as tb
        _record("Layer 3", "import uefn_tools", True)
    except Exception as e:
        _record("Layer 3", "import uefn_tools", False, str(e))
        return
    ...
    from uefn_tools import tools as _tools_pkg
...
def _layer_mcp() -> None:
    ...
    try:
        from uefn_tools.tools import mcp_bridge
...
def _layer_dashboard() -> None:
    ...
    try:
        # Dashboard removed
        _record("Layer 5", "uefn_toolsDashboard importable", True)
    except Exception as e:
        _record("Layer 5", "uefn_toolsDashboard importable", False, str(e))
...
def _layer_verse_book() -> None:
    ...
    # smoke_test.py lives at Content/Python/uefn_tools/ — go 3 levels up to project root
...
def _summary() -> None:
    ...
    _out(f"\n{'═' * 54}")
    _out(f"  uefn_tools SMOKE TEST — COMPLETE")
    _out(f"{'═' * 54}")
    ...
    if failed == 0:
        _out("\n  ✓ All systems healthy — uefn_tools is ready.", "info")
    else:
...
def run_smoke_test() -> bool:
    ...
    _out("\n[uefn_tools] Starting smoke test…")
    _layer_python()
    _layer_uefn()
    _layer_toolbelt()
    _layer_mcp()
