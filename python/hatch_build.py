...
def _find_repo_root(start: Path) -> Path | None:
    """Locate the directory that contains ``verse-api-pages-canonical/``.

    Walks up from *start* and also checks each level's ``docs/`` subdir so
    docs can live either at the repo root (legacy) or under ``python/docs/``.
    """
