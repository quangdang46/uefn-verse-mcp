...
def find_docs_root() -> Path | None:
    """Find the directory that contains ``verse-api-pages-canonical/``.

    Walks up from this file, supporting two layouts:
    - Docs sit at the repo root (legacy layout).
    - Docs sit under ``python/docs/`` (current main-branch layout).
    """
    ...
    for _ in range(10):
        if (current / "verse-api-pages-canonical").is_dir():
            return current
        nested = current / "docs"
        if (nested / "verse-api-pages-canonical").is_dir():
            return nested
