...
Here is the final jump to end the course. **Teleporter #6**, which has had its size expanded to make it easier to jump into. Leaping into it triggers the elimination arena portion of the map and puts a time limit on finishing the course as well as buying a weapon safely and finding a position in the arena. The event binding for the teleporter is configured once all the devices for the [30-second countdown](https://dev.epicgames.com/documentation/fortnite/parkour-elimination-4-elimination-arena-in-unreal-editor-for-fortnite) are in place.
