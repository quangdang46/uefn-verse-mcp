...
Check out the full [documentation](https://dev.epicgames.com/documentation/fortnite/fortnite-communities-in-fortnite) to learn more.
...
The actions are only responsive when UI mode is active (such as when the Verse UI takes input). The available input actions and mappings are currently under [Control Input UI](https://dev.epicgames.com/documentation/fortnite/verse-api/unrealenginedotcom/controlinput).
