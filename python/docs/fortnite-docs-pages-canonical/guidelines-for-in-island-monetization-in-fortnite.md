...
#### ✅ You are allowed to offer items in categories similar to those sold in the Fortnite Item Shop – aside from Outfits, Emotes, Cars, Trucks and Buses as noted above – as long as they have gameplay value and are not purely cosmetic.

You must accurately describe the gameplay value in the offer description and set the `ConsequentialToGameplay` field to true for these offers.

This applies when offered in exchange for V-Bucks and when offered in exchange for entitlements purchased with V-Bucks.

Here are some examples that are OK as long as you describe the gameplay value in the offer description and set the `ConsequentialToGameplay` field to true:
...
- A ‘frost gun’ that turns everything it shoots into ice.
- A dragon-shaped gun that does 25% more damage.
- An offer to unlock the pump shotgun when the player did not have access to it before.
...
- If you offer a “mystery item box” for purchase with V-Bucks, you must set the `PaidRandomItem` field in the entitlement to `true` and disclose the accurate numerical odds of obtaining each possible item prior to its purchase.
- If you offer a “luck boost” for purchase with V-Bucks that provides the player with a greater probability of receiving a random item in gameplay, you must set the `PaidRandomItem` field in the entitlement to `true` and disclose the accurate numerical odds of obtaining each possible item prior to its purchase.
- If you offer a key for purchase with V-Bucks that can only be used to open a "mystery item box”, you must set the `PaidRandomItem` field in the entitlement for the key to true and disclose the accurate numerical odds of obtaining each possible item in the mystery item box prior to its purchase.
- If you offer a key for purchase with V-Bucks that can be redeemed for various items including to open a “mystery item box”, you must use the `RestrictPaidRandomItems` function to prevent players without access  to Paid random items from using the key to open the "mystery item box".

#### ✅Do disclose the accurate numerical odds of obtaining paid random items.

If you offer paid random items, you must ensure players see accurate numerical odds of obtaining each paid random item prior to purchase.

This applies when random items are offered in exchange for V-Bucks or when offered in exchange for entitlements purchased with V-Bucks.

Here is how to correctly disclose the odds of paid random items:

- Include the odds directly in the offer description, or
- In the offer description, explain where the odds can be viewed and ensure those odds are visible to the player prior to purchase , or
- If you offer content for purchase that can be redeemed for various items including a random reward, disclose the odds elsewhere in the experience before a player can obtain the random rewards.
