## https://dev.epicgames.com/documentation/en-us/fortnite/using-fish-items-in-fortnite-creative

# Fish Items

Revive players and offer status buffs with these fishy items.

![Fish Items](https://dev.epicgames.com/community/api/documentation/image/7bbdaa52-a975-4f6a-86df-5e1dc6918516?resizing_type=fill&width=1920&height=335)

Use **Fish** [items](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#items) to offer aquatic-themed items that can heal, and that can apply [status buffs](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#buff) to players during combat gameplay. These items can be [placed](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#place) directly on your island or [granted](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#grant) using the devices found in [Registering Items](https://dev.epicgames.com/documentation/fortnite/using-fish-consumables-in-fortnite-creative#registeringitems).

Fish items include:

- Small Fry
- Stink Fish
- Zero Point Fish
- Spicy Fish
- Shield Fish
- Jellyfish
- Slurpfish
- Rift Fish
- Thermal Fish

## Finding and Placing Items

Click image to enlarge.

Clicking **Equip** will add the item to your [Equipment bar](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#equipmentbar). When you're back in Create mode, you can view any items you've [equipped](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#equip) by pressing the **F** key. You can select equipped items by scrolling your middle mouse button or by pressing the corresponding number on your keyboard.

From the [**Chest**](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#chesttab) tab, you can select either **Create Chest** or **Create Llama** to store the items in a [chest](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#chest) or a [llama](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#llama) for use [in-game](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#in-game).

Click image to enlarge.

Chests and llamas are a great way to offer item bundles to players. Selecting **Add To Chest** will add the item to the [**Chest** tab](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#chesttab). Each time you click **Add To Chest**, the item count will increase by one, shown as a yellow box on the Chest tab.

You can add up to fifteen items to the Chest tab. When it’s full, the **Add To Chest** button will disappear. To add more items, you first have to remove items from the Chest tab.

There are many ways for you to offer these items to players. Use any of the devices under the [Registering Items](https://dev.epicgames.com/documentation/fortnite/using-fish-consumables-in-fortnite-creative#registeringitems) section to offer or require these items during gameplay.

## Managing Items

![Fish Loadout Bar](https://d1iv7db44yhgxn.cloudfront.net/documentation/images/d32de053-338e-46c9-acb5-bbc2c8cdb36b/fish-loadout-bar.png)

You can manage these items when you are in [**Play** inventory](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#play-inventory). To access the **Play** inventory screen, press **Tab** and click **Play** in the top navigation bar. Here, you can swap item positions or drop them from your Equipment bar.

You cannot reposition or copy items with the phone tool. To delete an item from your inventory in Create mode, you will have to select either *Respawn* or *Back To Hub* from the *Menu*. When you do this, your inventory clears.

## Using Fish Items

Use these items to replenish damaged health and shield. It takes half a second to consume these items.

|  | Item | Usage |
| --- | --- | --- |
| Small Fry | **Small Fry** | Restores damaged health by 25 points. This item can heal up to 75 points of health. Players carry up to six of this item in a stack. |
| Stink Fish | **Stink Fish** | Restored damaged health by 20 points when consumed. This item can also be thrown toward enemies to release a cloud of gas that damages health. Players can carry up to six of this item in a stack. Stink Fish |
| Zero Point Fish | **Zero Point Fish** | Restores damaged health by 15 points. Players can carry up to three of this item in a stack. |
| Spicy Fish | **Spicy Fish** | Restores damaged health up to 15 points and grants a speed boost. Players can carry up to three of this item in a stack. Spicy Fish |
| Shield Fish | **Shield Fish** | Restores damaged shield by 50 points. Players can carry up to three of this item in a stack. |
| Jellyfish | **Jellyfish** | Consume for an explosion that grants 20 points of health to all players within its blast radius. Players can carry up to three of this item in a stack. |
| Slurpfish | **Slurpfish** | Restores damaged health by 40 points. If health is full, any remaining points will go toward shield. Players carry up to three of this item in a stack. |
| Rift Fish | **Rift Fish** | Restores damaged health by 15 points. Consuming this item also creates a rift that can be used by anyone before it closes. Players can carry up to two of this item in a stack. Rift Fish |
| Thermal Fish | **Thermal Fish** | Restores damaged shield by 15 points. Consuming this item also grants a 60-second thermal vision where players are highlighted in white. Players can carry up to two of this item in a stack. |

## Registering Items

![Registered Consumables](https://d1iv7db44yhgxn.cloudfront.net/documentation/images/f7d8492f-d2fc-445f-8304-f8feda54a8e6/registered-consumables.png)

You can drop items directly onto devices that can either hold or [grant](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#grant) items. You must [register](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#register) these items to save in the device's memory.

![Registering Consumables](https://d1iv7db44yhgxn.cloudfront.net/documentation/images/f07cf6c4-8af3-4b88-94e5-ca0a41eade3c/registering-consumables.png)

To register items to a device, stand directly on or immediately beside the device.

To register an item for this kind of device, follow these steps. You can also watch a [video tutorial](https://mediaspace.unrealengine.com/media/RegisteringCraftingConsumablesinFortniteCreative/1_zpmj3v0g) that shows you how to register items, both crafting and usable.

The compatible device will automatically register the dropped item. Compatible devices that can hold items include:

- **Vending Machine**
- **Team Settings & Inventory**
- **Class Designer**
- **Capture Item Spawner**
- **Capture Area**
- **Item Granter**
- **Item Spawner**
- **Conditional Button**
- **Elimination Manager**
- **Item Remover**

Use these devices to set up your own system for granting and spawning items onto your island.

## Using Items for Crafting

You can pair items with each other and with weapons to create the process of [crafting](https://dev.epicgames.com/documentation/fortnite/fortnite-creative-glossary#crafting), where players exchange items for another.

To do so, you can choose from the list of compatible devices above like the [**Conditional Button**](https://dev.epicgames.com/documentation/fortnite/using-conditional-button-devices-in-fortnite-creative) and [**Item Spawner**](https://dev.epicgames.com/documentation/fortnite/using-item-spawner-devices-in-fortnite-creative) to set up the crafting process.

For example, you can require [**Slurp Juice**](https://dev.epicgames.com/documentation/404) and **Small Fry** as items needed to craft **Slurpfish**.

Visit our [video tutorials](https://mediaspace.unrealengine.com/playlist/dedicated/208434573/1_gxu6mwv5/1_qfnz9w5c) to learn more about working with items and for tips to enhance gameplay.
